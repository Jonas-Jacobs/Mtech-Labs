...
func checkingAge(userInputAge: String) -> Int? {
    if let agechecker: Int = Int(userInputAge) {
        if agechecker > 18 {
            print("Welcome")
        } else if  agechecker < 18 {
            print("You arent old enough to use our app")
        }
        return agechecker
    } else {
...
checkingAge(userInputAge: "12")
checkingAge(userInputAge: "27")
checkingAge(userInputAge: "37e")
//:  Go back and update your function to return the age as an integer. Will your function always return a value? Make sure your return type accurately reflects this. Call the function and print the return value.
checkingAge(userInputAge: "something")
...
func purchases(SnackName: String) -> Double? {
    if let cost = prices[SnackName] {
        if let amount = stock[SnackName] {
            if amount >= 1 {
                if cost != nil {
                    print("\(SnackName) cost \(cost)")
                } else {
                    print("were out of \(SnackName)")
                }
            }
        }
    }
